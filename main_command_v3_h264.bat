...
ffmpeg -hwaccel cuda -i base_video.mp4 -i loop_video.mp4 -filter_complex "[0:v]format=gbrp[base];[1:v]format=gbrp[loop];[base][loop]blend=all_mode='lighten':all_opacity=1[out];[out]format=yuv420p[out_final_step_1]" -map "[out_final_step_1]" -c:v h264_nvenc -r 25 -color_primaries bt709 -color_trc bt709 -colorspace bt709 video_step_1.mp4

@REM Step 3: render final video (New way)
